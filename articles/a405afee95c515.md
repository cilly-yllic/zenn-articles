...
:::message
この記事は執筆当時（2024年）の構成です。その後この仕組みは、gcloud + Makefile での下準備も含めて設定ファイル一枚から CI で自動払い出しする形に作り直しました。現在の構成は [settings.yml 一枚で GCP / Firebase 環境を払い出す](https://zenn.dev/cilly/articles/config-driven-gcp-provisioning) に書いています。当時の記録として、本文はそのまま残します。
:::
